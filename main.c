...
// Semilla global para la funcion aleatorioa
unsigned int seed = 12345;

// Generar un numero aleatorio
int generarAleatorio() {
  // Parametros para LCG
  unsigned int a = 16645;
  unsigned int c = 10139042;
  unsigned int m = 4096; // 2^12

  // Calcular (a * seed + c) %
  unsigned long long temp = (unsigned long long)a * seed + c;
  seed = temp % m;

  // Escalar el resultado a [10, 50]
  int numero = (seed % 41) + 10;
...
void generarNumeroAleatorio() {
  int numero = generarAleatorio(); // Genera número entre 10 y 50
